...
3. **Customer Scope ([setup-customer-sa.sh](setup-customer-sa.sh)):** Provisions resources strictly within GDC-native customer tenant boundaries.
...
- **`../../000-SETUP/base-customer-identity.yaml`**: The parameterized declarative Kubernetes manifest applied to the Global API Cluster to bootstrap the logical Service Accounts (`test-setup-sa`, `test-runner-003-sa`) and GDC-native IAM bindings.
...
sed -e "s/\\${TEST_CASE_NO}/003/g" -e "s/\\${IAC_PROJECT}/iac-root/g" ../../000-SETUP/base-customer-identity.yaml | kubectl --context "$GLOBAL_API_CONTEXT" delete -f -
rm -f .kubeconfig-sa
```
...
graph TD
    subgraph "1. Logical Personas (Identity Layer)"
        ...
        SA_R["Test Runner SA<br/>(test-runner-003-sa)"]
    end
