# 005-AO-KMS-CREATE-REFACTORED

This test case demonstrates the creation of HSM-backed encryption keys (KMS) in GDC using a refactored, persona-based approach.

## Overview

- **Stage 1 (PA)**: The Platform Admin (using `ioc-test-pa`) creates the project, grants the `kms-admin` role to the AO, and binds the project to a user cluster.
- **Stage 2 (AO)**: The Application Operator (using `ioc-test-ao`) provisions the HSM-backed AEAD keys within the pre-created project.
...
- **Isolation**: By using a unique `PROJECT_NAME` (e.g., `pt-005-kms-create`), all resources for this test are isolated within that specific namespace.
...
1.  **CLI**: Verify the KMS keys:
    ```bash
    kubectl --context ${ADMIN_CLUSTER_CONTEXT} get aeadkeys.kms.gdc.goog -n ${PROJECT_NAME}
    ```
