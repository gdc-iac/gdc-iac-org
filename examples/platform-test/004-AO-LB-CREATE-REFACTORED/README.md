# 004-AO-LB-CREATE-REFACTORED

This test case demonstrates the creation of an Internal Load Balancer in GDC using a refactored, persona-based approach.

## Overview

- **Stage 1 (PA)**: The Platform Admin (using `ioc-test-pa`) creates the project, grants IAM roles (`load-balancer-admin`, `project-vm-admin`) to the AO, and binds the project to a user cluster.
- **Stage 2 (AO)**: The Application Operator (using `ioc-test-ao`) provisions the backend Virtual Machine and the Load Balancer suite (Backend Service, Health Check, Forwarding Rule).
...
- **Isolation**: By using a unique `PROJECT_NAME` (e.g., `pt-004-lb-create`), all resources for this test are isolated within that specific namespace.
...
1.  **CLI**: Verify the Load Balancer resources:
    ```bash
    kubectl --context ${ADMIN_CLUSTER_CONTEXT} get forwardingruleinternals,backendservices,healthchecks -n ${PROJECT_NAME}
    ```
