-- Operational Readiness & Multi-Domain Sensor Telemetry Database Schema
-- Mission: Operation Vanguard Shield (Multi-Domain Decision Support)

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 1. Multi-Domain Sensor Telemetry (Kafka Ingestion Target)
CREATE TABLE IF NOT EXISTS sensor_telemetry (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    event_timestamp TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    domain VARCHAR(50) NOT NULL, -- 'LAND', 'AIR', 'SEA', 'SPACE', 'CYBER'
...
-- 2. Military Units (Force Structure & Readiness)
CREATE TABLE IF NOT EXISTS military_units (
    unit_id VARCHAR(50) PRIMARY KEY,
    unit_name VARCHAR(150) NOT NULL,
    branch VARCHAR(50) NOT NULL, -- 'Army', 'Air Force', 'Navy', 'Marines', 'Space Force', 'Cyber Command'
    sector VARCHAR(50) NOT NULL,
    base_location VARCHAR(100) NOT NULL,
    readiness_rating VARCHAR(10) NOT NULL, -- 'C1' (Fully Ready), 'C2' (Substantially Ready), 'C3' (Marginally Ready), 'C4' (Not Ready)
    commander VARCHAR(100) NOT NULL,
    personnel_count INT NOT NULL,
    operational_status VARCHAR(50) NOT NULL
);

-- 3. Equipment Inventory & Maintenance
...
-- 4. Fuel, Ammunition & Supply Stocks
...
-- ============================================================================
-- SEED DATA: Operation Vanguard Shield
-- ============================================================================
...
-- Sensor Telemetry (Sample Multi-Domain Ingestion Seed)
INSERT INTO sensor_telemetry (domain, sensor_id, sector, threat_level, latitude, longitude, title, summary, raw_payload) VALUES
('LAND', 'SECTOR9-AGS-044', 'Sector 9', 'HIGH', 36.8124, -115.9123, 'Acoustic Ground Sensor Tripwire', 'Heavy tracked vehicle signatures detected moving eastward towards Waypoint Echo on Route 9.', '{"frequency_hz": 42.5, "estimated_axles": 6, "confidence": 0.89}'),
('AIR', 'RADAR-AWACS-7', 'Sector 9', 'MEDIUM', 37.1021, -115.7891, 'Unidentified Medium-Altitude Track', 'Intermittent radar contact 14,000 ft MSL moving at 180 kts. Squawk code unassigned.', '{"squawk": "7700_ALT", "heading_deg": 142, "rcs_sqm": 2.4}'),
('SEA', 'AIS-LITTORAL-02', 'Sector 9', 'LOW', 36.4512, -116.1245, 'Commercial Cargo Vessel Transit', 'Container vessel "Star Navigator" verified cargo manifest. Arriving Port Campion.', '{"mmsi": 367412000, "speed_knots": 14.2, "flag": "US"}'),
('SPACE', 'SATCOM-PASS-91', 'Sector 9', 'LOW', 37.3500, -116.2000, 'Synthetic Aperture Radar (SAR) Downlink', 'Orbital SAR pass confirmed no thermal signatures on Highway 4 North Bypass.', '{"satellite": "USA-314", "resolution_meters": 0.5, "swath_km": 40}'),
('CYBER', 'IDS-FIREWALL-FOB-A', 'Sector 9', 'CRITICAL', 36.8100, -115.9100, 'Industrial Control System Port Scan', 'Coordinated TCP SYN flood against FOB Alpha automated fuel pump distribution SCADA system.', '{"source_ip": "198.51.100.44", "target_port": 502, "protocol": "Modbus/TCP"}'),
('LAND', 'UAV-RECON-CAMERA-3', 'Sector 9', 'HIGH', 36.8201, -115.9080, 'Thermal Bridge Abutment Inspection', 'Bridge at Route 9 Waypoint Echo shows fresh earthwork and concrete curing equipment.', '{"infra_type": "box_girder", "traffic_capable": false, "repair_hours_remaining": 14}');
