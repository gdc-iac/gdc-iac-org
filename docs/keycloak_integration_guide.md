...
* Change **Valid Redirect URIs** from `*` physically to: `https://app.gdc.local/*` (or whatever your absolute physical ingress URL is).
...
## 6. Pattern 13: GDC Developer Environment (`gdc-dev`) OIDC & RBAC Setup

Pattern 13 implements enterprise Single Sign-On (SSO) and Role-Based Access Control (RBAC) via Keycloak OIDC, separating software developer workspace access from administrative platform governance across two dedicated ports:

| Access Persona | Dedicated Ingress Port | Assigned Keycloak Roles | Session Cookie | Target Endpoints |
| :--- | :--- | :--- | :--- | :--- |
| **Developer** | **Port 80 / 8080** | `developer`, `gdc-developer` | `dev_auth_user` | `/`, `/instances/*`, in-pod terminal, AI Co-Pilot |
| **Administrator** | **Port 8081** | `admin`, `gdc-admin`, `dev-admin` | `admin_auth_user` | `/admin`, `/api/admin/metrics`, `/admin/telemetry/*` |

### 6.1 Step-by-Step Keycloak Realm Configuration

Open the Keycloak Admin Console (`https://keycloak.gdc.local/admin` or `http://localhost:8080/auth/admin`) and perform the following configuration:

#### 1. Create Dedicated Realm
* Click Realm dropdown (top left) -> **Create Realm**.
* **Realm Name:** `gdc-dev-realm`
* **Enabled:** `ON`
* Click **Create**.

#### 2. Define Realm Roles
* Navigate to **Realm Roles** -> Click **Create Role**:
  1. **Role: `admin`**
     * Description: `Full platform administrator clearance for GDC Dev Admin Console (Port 8081), session eviction, live metrics, and AI policy controls.`
     * Also create alias roles `gdc-admin` and `dev-admin` if adhering to organizational naming taxonomies.
  2. **Role: `developer`**
     * Description: `Standard software developer access for GDC Dev Workspaces (Port 80 / 8080), Monaco editor, and AI Co-Pilot.`
     * Also create alias role `gdc-developer`.

#### 3. Create Admin & Developer User Accounts
Create two accounts with distinct role memberships to validate dual-port access and session isolation:

* **Administrator Account (`admin@gdc.local`):**
  1. Navigate to **Users** -> Click **Add user**.
  2. **Username:** `admin@gdc.local` (or `admin`)
  3. **Email:** `admin@gdc.local` | **Email Verified:** `ON` | **Enabled:** `ON`
  4. Click **Create** / **Save**.
  5. Go to **Credentials** tab -> Click **Set password**:
     * Password: `AdminSecure2026!`
     * **Temporary:** `OFF`
     * Click **Save**.
  6. Go to **Role mapping** tab -> Click **Assign role**:
     * Select `admin` and `gdc-admin` -> Click **Assign**.

* **Developer Account (`developer@gdc.local`):**
  1. Navigate to **Users** -> Click **Add user**.
  2. **Username:** `developer@gdc.local` (or `oidc-developer@gdc.local`)
  3. **Email:** `developer@gdc.local` | **Email Verified:** `ON` | **Enabled:** `ON`
  4. Click **Create** / **Save**.
  5. Go to **Credentials** tab -> Click **Set password**:
     * Password: `DevSecure2026!`
     * **Temporary:** `OFF`
     * Click **Save**.
  6. Go to **Role mapping** tab -> Click **Assign role**:
     * Select `developer` (or `gdc-developer`) -> Click **Assign**.

#### 4. Register Client `gdc-dev-client`
* Navigate to **Clients** -> Click **Create client**:
  * **Client Type:** `OpenID Connect`
  * **Client ID:** `gdc-dev-client`
  * **Name:** `GDC Developer Environment Client`
  * Click **Next**.
* **Capability Config:**
  * **Client Authentication:** `OFF` (Public PKCE client for direct browser SPA / Landing Page flows) or `ON` (Confidential if using Gateway edge proxy).
...
* **Login Settings & Redirect URIs:**
  * **Valid Redirect URIs:**
    ```text
    https://dev.gdc.local/*
    https://dev.gdc.local:8081/*
    http://localhost:8080/*
    http://localhost:8081/*
    https://8080-*.workstations.cloud.google/*
    https://8081-*.workstations.cloud.google/*
    ```
  * **Valid Post-Logout Redirect URIs:** `+`
  * **Web Origins:** `+` (or `*` for emulation development).
  * Click **Save**.

#### 5. Configure Client Scopes & Role Token Claim Mappers
Ensure Keycloak injects the user's roles into token claims so that upstream reverse proxies and the backend operator can evaluate RBAC:
1. Navigate to **Client scopes** -> Select `roles` (or create a dedicated client scope `gdc-dev-roles`).
2. Go to **Mappers** tab -> Click **Add mapper** -> **By configuration** -> Select **User Realm Role**:
   * **Name:** `realm-roles`
...
#### 6. Upstream Gateway Header Forwarding Configuration
When deploying behind Kubernetes Gateway API (Envoy Gateway / OAuth2-Proxy), configure header injection on authenticated routes:
* **`X-Forwarded-User`** or **`X-User-ID`**: Set to `$jwt.preferred_username` or `$jwt.email` (e.g. `admin@gdc.local`).
* **`X-Forwarded-Groups`** or **`X-User-Roles`**: Set to `$jwt.roles` (e.g. `admin,gdc-admin` or `developer`).

The backend `gdc-dev-landing-page` and `gdc-dev-operator` services read these headers via environment variable keys (`HEADER_USER_KEY` and `HEADER_ROLES_KEY`). When running in dual-port mode, browser cookies are completely partitioned (`dev_auth_user` on Port 8080 vs `admin_auth_user` on Port 8081), enabling seamless concurrent administration and development without session collisions.
