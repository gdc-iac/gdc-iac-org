# Stage 5: Blueprint Patterns (`5-patterns`)
...
- **User Cluster Workloads (`Stage 5: 5-patterns`)**: Kubernetes `Deployment`, `StatefulSet`, `Service`, `Gateway`, `HTTPRoute`, `CronJob`, and `NetworkPolicy` resources are deployed to the target User/Standard Cluster (`kube_context_cluster`) with `apps.enabled: true`.
