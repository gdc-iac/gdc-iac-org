# gdc-project-bindings

A Helm chart for managing ProjectBindings in Google Distributed Cloud Hosted environments.
...
```bash
helm install my-project-bindings ./gdc-project-bindings -f values.yaml
```
...
| --- | --- | --- | --- |
| `namespace` | The namespace for the `ProjectBinding` metadata (typically `"platform"`). | `"platform"` | No |
| `projectBindings` | List of project cluster bindings to create. | `[]` | No |
| `projectBindings[].clusterName` | The target user cluster name. | `""` | **Yes** if bindings provided |
| `projectBindings[].projectName` | The name of the GDC project to bind. | `""` | **Yes** if bindings provided |
